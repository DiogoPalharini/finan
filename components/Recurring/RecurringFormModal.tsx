...
const RecurringFormModal: React.FC<RecurringFormModalProps> = ({ visible, onClose, recorrenciaId, onSuccess }) => {
  const { user } = useAuth();
  ...
  const loadRecorrencia = async () => {
    try {
      setLoading(true);
      const recorrencia = await getRecorrencia(user!.uid, recorrenciaId!);
      if (recorrencia) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Prepara os dados da recorrência
      const recorrenciaData: Omit<Recorrencia, 'id' | 'status' | 'createdAt' | 'updatedAt'> = {
        tipo,
        valor: valorNumerico,
        descricao: descricao.trim(),
        categoria,
        diaRecorrencia: dataInicio.getDate(),
        dataInicio: dataInicio.toISOString()
      };

      // Adiciona a data de término apenas se estiver definida
      if (temDataFim && dataFim) {
        recorrenciaData.dataFim = dataFim.toISOString();
      }

      if (recorrenciaId) {
        await updateRecorrencia(user.uid, recorrenciaId, recorrenciaData);
      } else {
        ...
        <View style={styles.modalContainer}>
          ...
          >
            ...
            <View style={styles.headerContent}>
              <Text style={styles.headerTitle}>
                {recorrenciaId ? 'Editar Recorrência' : 'Nova Recorrência'}
              </Text>
              <Text style={styles.headerSubtitle}>
                {recorrenciaId ? 'Modifique os dados abaixo' : 'Configure sua transação automática'}
              </Text>
            </View>
          </LinearGradient>
          ...
          >
            ...
            >
              ...
              {/* Data Card */}
              <View style={styles.card}>
                <Text style={styles.cardTitle}>Data de Início</Text>
                <TouchableOpacity
                  style={styles.input}
                  onPress={() => {
                ...
                <View style={styles.dataFimContainer}>
                  ...
                  >
                    ...
                    <Text style={styles.checkboxLabel}>Definir data de término</Text>
                  </TouchableOpacity>
                ...
                >
                  ...
                  >
                    ...
                    <Text style={styles.saveButtonText}>
                      {recorrenciaId ? 'Salvar Alterações' : 'Criar Recorrência'}
                    </Text>
                  </LinearGradient>
                </TouchableOpacity>
              </View>
